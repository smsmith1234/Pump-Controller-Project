...
#define PRESSURE_DATA_PIN 5
#define PUMP_RUN_PIN 6
#define LED_PIN 5
#define MIN_PRESSURE 10
#define LP_TURN_ON 28
...
if(status < 3){  // Pump in normal operations
    ...
    else{  // Pump is on - track temperature deviation
    ...
    // Determine operational condition then command pump and update status
    if (pressure > maxPressure){  // Pump is in overpressure condition - turn pump off and generate error
        ...
        //delay(3000);  // Let pressure build a bit *** Removed 13 Nov 16 - caused over pressure alarm
        digitalWrite(PUMP_RUN_PIN, LOW);
